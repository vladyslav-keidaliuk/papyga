...
from phrases import lists_phrases, mountain_phrases, mine_phrases, get_text_with_link
...
def check_timer(message, user_id, size, stop_timer, what_timer):
    try:
        ...
        if now < stop_timer:
            ...
            text = get_text_with_link(message) + ", ти рано зайшов(-ла), зможеш спробувати знов після " + stop_timer_str
            return text
        elif now >= stop_timer:
            if what_timer == 0:
                new_try_qt(user_id)
